...
static void mg3P(std::vector<std::vector<std::vector<std::vector<double> > > > &u, std::vector<std::vector<std::vector<double> > > &v, std::vector<std::vector<std::vector<std::vector<double> > > > &r, double a[4], double c[4], int n1, int n2, int n3, int k);
static void psinv(std::vector<std::vector<std::vector<double> > > &r, std::vector<std::vector<std::vector<double> > > &u, int n1, int n2, int n3, double c[4], int k);
static void resid(std::vector<std::vector<std::vector<double> > > &u, std::vector<std::vector<std::vector<double> > > &v, std::vector<std::vector<std::vector<double> > > &r, int n1, int n2, int n3, double a[4], int k);
static void rprj3(std::vector<std::vector<std::vector<double> > > &r, int m1k, int m2k, int m3k, std::vector<std::vector<std::vector<double> > > &s, int m1j, int m2j, int m3j, int k);
static void interp(std::vector<std::vector<std::vector<double> > > &z, int mm1, int mm2, int mm3, std::vector<std::vector<std::vector<double> > > &u, int n1, int n2, int n3, int k);
static void norm2u3(std::vector<std::vector<std::vector<double> > > &r, int n1, int n2, int n3, double *rnm2, double *rnmu, int nx, int ny, int nz);
static void rep_nrm(std::vector<std::vector<std::vector<double> > > &u, int n1, int n2, int n3, char *title, int kk);
static void comm3(std::vector<std::vector<std::vector<double> > > &u, int n1, int n2, int n3);
static void zran3(std::vector<std::vector<std::vector<double> > > &z, int n1, int n2, int n3, int nx, int ny, int k);
static void showall(std::vector<std::vector<std::vector<double> > > &z, int n1, int n2, int n3);
static double power(double a, int n);
static void bubble(double ten[M][2], int j1[M][2], int j2[M][2], int j3[M][2], int m, int ind);
static void zero3(std::vector<std::vector<std::vector<double> > > &z, int n1, int n2, int n3);
...
int main(int argc, char *argv[]) {
	...
	timer_clear(T_BENCH);
	timer_clear(T_INIT);
	if(TIMERS_ENABLED == TRUE) timer_clear(T_STL);
	...
	std::vector<std::vector<std::vector<std::vector<double> > > > u (lt+1);
	...
	std::vector<std::vector<std::vector<double> > > v (m3[lt], std::vector<std::vector<double> > (m2[lt], std::vector<double> (m1[lt])));
	
	std::vector<std::vector<std::vector<std::vector<double> > > > r (lt+1);
...
static void psinv( std::vector<std::vector<std::vector<double> > > &r, std::vector<std::vector<std::vector<double> > > &u, int n1, int n2, int n3, double c[4], int k) {
	...
	std::vector<int> v(n3-2);
	std::iota(v.begin(), v.end(), 1);
	
	std::for_each(pstl::execution::par, v.begin(), v.end(), [&r, &u, &n1, &n2, &c](int i3)
	{
...
static void resid( std::vector<std::vector<std::vector<double> > > &u, std::vector<std::vector<std::vector<double> > > &v, std::vector<std::vector<std::vector<double> > > &r, int n1, int n2, int n3, double a[4], int k ) {
	...
	std::vector<int> c(n3-2);
	std::iota(c.begin(), c.end(), 1);
	
	std::for_each(pstl::execution::par, c.begin(), c.end(), [&u, &v, &r, &n1, &n2, &a](int i3)
	{
...
static void rprj3( std::vector<std::vector<std::vector<double> > > &r, int m1k, int m2k, int m3k, std::vector<std::vector<std::vector<double> > > &s, int m1j, int m2j, int m3j, int k ) {
	...
	std::vector<int> v(m3j-2);
	std::iota(v.begin(), v.end(), 1);
	
	std::for_each(pstl::execution::par, v.begin(), v.end(), [&r, &s, &m1j, &m2j, &d1, &d2, &d3](int j3)
	{
...
static void interp( std::vector<std::vector<std::vector<double> > > &z, int mm1, int mm2, int mm3, std::vector<std::vector<std::vector<double> > > &u, int n1, int n2, int n3, int k ) {
	...
	*/
	...
	if ( n1 != 3 && n2 != 3 && n3 != 3 ) {
		...
		std::vector<int> v(mm3-1);
		std::iota(v.begin(), v.end(), 0);
		
		std::for_each(pstl::execution::par, v.begin(), v.end(), [&z, &u, &mm1, &mm2](int i3)
		{
			
			std::vector<double> z1 (M);
			std::vector<double> z2 (M);
			std::vector<double> z3 (M);
		...
		std::vector<int> v1(mm3-d3);
		std::iota(v1.begin(), v1.end(), d3);
		
		std::for_each(pstl::execution::par, v1.begin(), v1.end(), [&z, &u, &d1, &d2, &d3, &t1, &t2, &t3, &mm1, &mm2](int i3)
		{
		...
		std::vector<int> v2(mm3);
		std::iota(v2.begin(), v2.end(), 1);
		
		std::for_each(pstl::execution::par, v2.begin(), v2.end(), [&z, &u, &d1, &d2, &d3, &t1, &t2, &t3, &mm1, &mm2](int i3)
		{
			for (int i2 = d2; i2 <= mm2-1; i2++) {
				for (int i1 = d1; i1 <= mm1-1; i1++) {
...
static void norm2u3(std::vector<std::vector<std::vector<double> > > &r, int n1, int n2, int n3, double *rnm2, double *rnmu, int nx, int ny, int nz) {
	...
	}, [&n1, &n2](std::vector<std::vector<double> > &r) -> std::pair<double, double>{
	...
	std::for_each(pstl::execution::par, v.begin(), v.end(), [&r, &n1, &n2, &p_a, &p_s, &my_m](int i3)
	{
		double p_s_tbb = 0.0, p_a_tbb = 0.0;
		double tmp;
...
static void comm3(std::vector<std::vector<std::vector<double> > > &u, int n1, int n2, int n3) {
	...
	// axis = 1
	/*
	std::vector<int> v1(n3-2);
	std::iota(v1.begin(), v1.end(), 1);
	
	std::for_each(pstl::execution::par, v1.begin(), v1.end(), [&u, &n2, &n1](int i3)
	{
		for (int i2 = 1; i2 < n2-1; i2++) {
	...
	std::for_each(pstl::execution::par, &u[1], &u[n3-1], [&n2, &n1](std::vector<std::vector<double> > &u)
	{
		for (int i2 = 1; i2 < n2-1; i2++) {
			u[i2][n1-1] = u[i2][1];
			u[i2][0] = u[i2][n1-2];
	...
	std::for_each(pstl::execution::par, v2.begin(), v2.end(), [&u, &n3, &n1](int i2)
	{
		for (int i1 = 0; i1 < n1; i1++) {
...
static void zero3(std::vector<std::vector<std::vector<double> > > &z, int n1, int n2, int n3) {
	...
	std::for_each(pstl::execution::par, &z[0], &z[n3], [&n1, &n2](std::vector<std::vector<double> > &z)
	{
	...
	std::for_each(pstl::execution::par, v.begin(), v.end(), [&z, &n1, &n2](int i3)
	{
		for (int i2 = 0; i2 < n2; i2++) {
			for (int i1 = 0; i1 < n1; i1++) {
				z[i3][i2][i1] = 0.0;
