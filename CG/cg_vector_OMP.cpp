...
int main(int argc, char **argv)
{
	...
	for (it = 1; it <= 1; it++) {
		...
		norm_temp11 = std::transform_reduce(pstl::execution::par, x.begin()+1, x.begin()+ lastcol - firstcol + 2 , z.begin()+1, 0.0);
		norm_temp12 = std::transform_reduce(pstl::execution::par, z.begin()+1, z.begin()+ lastcol - firstcol + 2 , z.begin()+1, 0.0);
		...
		std::transform(pstl::execution::par, z.begin()+1, z.begin()+ lastcol - firstcol + 2, x.begin()+1, [norm_temp12](double z) -> double {return norm_temp12*z;});
...
c---------------------------------------------------------------------*/
{
	...
	std::fill(pstl::execution::par, q.begin() + 1, q.begin() + naa + 2, 0.0);
	std::fill(pstl::execution::par, z.begin() + 1, z.begin() + naa + 2, 0.0);
	std::fill(pstl::execution::par, w.begin() + 1, w.begin() + naa + 2, 0.0);
	
	std::copy(pstl::execution::par, x.begin() + 1, x.begin() + naa + 2, r.begin() + 1);
	std::copy(pstl::execution::par, r.begin() + 1, r.begin() + naa + 2, p.begin() + 1);
	...
	if(TIMER_ENABLED == TRUE) timer_start(2);
	rho = std::transform_reduce(pstl::execution::par, r.begin() + 1, r.begin() + lastcol-firstcol+2, r.begin() + 1, 0.0);
	if(TIMER_ENABLED == TRUE) timer_stop(2);
	...
	for (cgit = 1; cgit <= cgitmax; cgit++) {
		...
		*/
		...
		if(TIMER_ENABLED == TRUE) timer_start(2);
		std::copy(pstl::execution::par, w.begin() + 1, w.begin() + lastcol - firstcol + 2, q.begin() + 1);
		if(TIMER_ENABLED == TRUE) timer_stop(2);
		/*--------------------------------------------------------------------
		c  Clear w for reuse...
		c-------------------------------------------------------------------*/
		...
		if(TIMER_ENABLED == TRUE) timer_start(2);
		std::fill(pstl::execution::par, w.begin() + 1, w.begin() + lastcol - firstcol + 2, 0.0);
		if(TIMER_ENABLED == TRUE) timer_stop(2);
		/*--------------------------------------------------------------------
		c  Obtain p.q
		c-------------------------------------------------------------------*/
		...
		if(TIMER_ENABLED == TRUE) timer_start(2);
		d = std::transform_reduce(pstl::execution::par, p.begin() + 1, p.begin() + lastcol - firstcol + 2, q.begin() + 1, 0.0);
		if(TIMER_ENABLED == TRUE) timer_stop(2);
		/*--------------------------------------------------------------------
		...
		if(TIMER_ENABLED == TRUE) timer_start(2);
		std::transform(pstl::execution::par, p.begin() + 1, p.begin() + lastcol - firstcol + 2, z.begin() + 1, z.begin() + 1, [alpha](double p, double z) -> double {return z + alpha*p;});
		std::transform(pstl::execution::par, q.begin() + 1, q.begin() + lastcol - firstcol + 2, r.begin() + 1, r.begin() + 1, [alpha](double q, double r) -> double {return r - alpha*q;});
		if(TIMER_ENABLED == TRUE) timer_stop(2);
		/*---------------------------------------------------------------------
		c  rho = r.r
		c  Now, obtain the norm of r: First, sum squares of r elements locally...
		...
		if(TIMER_ENABLED == TRUE) timer_start(2);
		std::transform(pstl::execution::par, p.begin() + 1, p.begin() + lastcol - firstcol + 2, r.begin()+1, p.begin()+1, [beta](double p, double r) -> double {return r + beta*p;});
		if(TIMER_ENABLED == TRUE) timer_stop(2);
	...
	if(TIMER_ENABLED == TRUE) timer_start(2);
	std::copy(pstl::execution::par, w.begin() + 1, w.begin() + lastcol - firstcol + 2, r.begin()+1);
	if(TIMER_ENABLED == TRUE) timer_stop(2);
	/*--------------------------------------------------------------------
