...
int main(int argc, char **argv) {
	...
	*/
	...
	int nhardware = std::thread::hardware_concurrency();
	int res[nhardware];

	for(int i = 0; i < nhardware; i++) res[i]=0;
	
	mystd::for_each(pstl::execution::par, &v[0], &v[np], [&an, &sx, &sy, &k_offset, &my_m, &res](int k)
	{
	...
	for(int i = 0; i < nhardware; i++) sum += res[i];
