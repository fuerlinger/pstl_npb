...
int main(int argc, char **argv) {
	...
	end_res = std::reduce(pstl::execution::seq, v.begin(), v.end(), end_res);
	
	auto end = std::chrono::high_resolution_clock::now();

	printf(" std::reduce() in serial:                 %llu.%06llu seconds\n",
	...
	for(int i = 2; i <= n; i+=2) {
		...
		printf(" std::reduce() with %02d threads:     %llu.%06llu seconds\n", i, 
		std::chrono::duration_cast<std::chrono::seconds>(end - begin).count(), 
		std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() % 1000000);
