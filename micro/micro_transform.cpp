...
int main(int argc, char **argv) {
	...
	std::transform(pstl::execution::seq, v.begin(), v.end(), v.begin(), [](double a) ->double{
	...
	printf(" std::transform() in serial:                 %llu.%06llu seconds\n",
	...
	for(int i = 2; i <= n; i+=2) {
		...
		printf(" std::transform() with %02d threads:     %llu.%06llu seconds\n", i, 
		std::chrono::duration_cast<std::chrono::seconds>(end - begin).count(), 
		std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() % 1000000);
