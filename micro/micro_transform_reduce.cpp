...
int main(int argc, char **argv) {
	...
	end_res = std::transform_reduce(pstl::execution::seq, v.begin(), v.end(), end_res, std::plus<double>(), [](double a) ->double{
	...
	printf(" std::transform_reduce() in serial:                 %llu.%06llu seconds\n",
	...
	for(int i = 2; i <= n; i+=2) {
		...
		printf(" std::transform_reduce() with %02d threads:     %llu.%06llu seconds\n", i, 
		std::chrono::duration_cast<std::chrono::seconds>(end - begin).count(), 
		std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() % 1000000);
